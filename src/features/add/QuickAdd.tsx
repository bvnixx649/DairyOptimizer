...
const MODES: { value: QuickMode; label: string }[] = [
  ...
  { value: 'event', label: 'นัดหมาย' },
]

export function QuickAdd({ sheet }: { sheet: QuickSheet }) {
  const [mode, setMode] = useState<QuickMode>(sheet.mode)
  return (
    <Sheet label="เพิ่ม" title={<Segmented value={mode} options={MODES} onChange={setMode} label="ประเภท" />}>
...
function TaskQuick({ sheet }: { sheet: QuickSheet }) {
  ...
  return (
    ...
    >
      <input className="big-input" autoFocus placeholder="ต้องทำอะไร" value={title} maxLength={180} onChange={(e) => setTitle(e.target.value)} enterKeyHint="done" />

      <div className="q-group">
        <span className="q-label">ส่ง</span>
        <div className="chips">
      ...
      <div className="prop-list">
        <div className="prop">
          ...
          <Switch on={priv} onChange={setPriv} label="งานส่วนตัว" />
        </div>
      </div>

      <button className="btn btn-primary btn-block" disabled={!title.trim()}>
        {sheet.start ? `เพิ่มและจองเวลา ${sheet.start}` : 'เพิ่มงาน'}
      </button>
    </form>
  )
}
...
function TxQuick({ type }: { type: TxType }) {
  ...
  const submit = () => {
    ...
    close()
    notify(`${type === 'expense' ? 'จ่าย' : 'รับ'} ฿${baht(satang)} · ${category}`, () => remove('txs', [id]))
  }
    ...
    <div className="stack tx-quick" style={{ '--c': type === 'expense' ? 'var(--text)' : 'var(--income)' } as CSSProperties}>
      ...
      <div className="chips tx-extras">
        ...
        <button type="button" className="chip" aria-pressed={showNote || !!note} onClick={() => setShowNote(true)}>
          <StickyNote size={15} /> โน้ต
        </button>
      </div>
      {(showNote || note) && (
        <label className="field">
          <input autoFocus placeholder="ซื้ออะไร / จากไหน" value={note} maxLength={150} onChange={(e) => setNote(e.target.value)} />
        </label>
      )}

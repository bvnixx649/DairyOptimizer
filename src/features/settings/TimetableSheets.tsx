...
export function ClassEditSheet({ id, subjectId }: { id?: string; subjectId?: string }) {
  ...
  const save = () => {
    ...
    put('classes', c)
    close()
    notify('บันทึกตารางแล้ว')
  }

  return (
    <Sheet
      title={existing ? 'แก้ไขคาบเรียน' : 'เพิ่มคาบเรียน'}
      footer={
        <>
          {existing && (
            <button
              ...
              onClick={() => {
                ...
                close()
                notify('ลบคาบแล้ว', () => restore(snaps))
              }}
            >
              <Trash2 size={18} />
            </button>
    ...
    >
      <div className="stack">
        ...
        <div className="prop-list">
          ...
          <label className="prop">
            <MapPin size={18} />
            <span className="prop-label">ห้อง</span>
            <input style={{ textAlign: 'right', width: 120 }} placeholder="SC2-307" value={room} maxLength={80} onChange={(e) => setRoom(e.target.value)} />
          </label>
        </div>
        {start >= end && <div className="note warn">เวลาจบต้องหลังเวลาเริ่ม</div>}
        {clash && (
          <div className="note warn">
            <AlertTriangle size={16} />
            ชนกับ {byId.get(clash.subjectId)?.short} {clash.start}–{clash.end}
          </div>
        )}
      </div>
    </Sheet>
...
export function SubjectEditSheet({ id }: { id?: string }) {
  ...
  const save = () => {
    ...
    put('subjects', s)
    close()
    notify('บันทึกวิชาแล้ว')
  }

  return (
    <Sheet
      title={existing ? 'แก้ไขวิชา' : 'วิชาใหม่'}
      footer={
        <>
          {existing && (
            <button
              ...
              onClick={() => {
                ...
                close(2)
                notify('ลบวิชาแล้ว', () => restore(snaps))
              }}
            >
              <Trash2 size={18} />
            </button>
